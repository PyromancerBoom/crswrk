...
function convertToSound(note){
    const midi_note_to_frequency(letter_name_to_midi_note(note));
}

const notes = list("C4", "E4", "G4");

function playSong(listOfNotes, instrument, duration){
    return consecutively(map(convertToSound(listOfNotes), listOfNotes));
}
...
play(playSong(notes, piano, 1));
play(C_maj);
play(playInstrument("A4", cello, 1));