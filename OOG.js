...
// O(n^2)
function g(n) {
    return n === 0 ? 0 : head(enum_list(1, n)) + g(n - 1);
}

// O(2^n)
function h(n) {
    return n === 0 ? 0 : 1 + h(n - 1) + h(n - 1);
}
...
function m(n) {
    return n === 0 ? null : accumulate(pair, null, reverse(append(enum_list(1, 1000), list(null))));
}
